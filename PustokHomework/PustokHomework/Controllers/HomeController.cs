    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            HomeViewModel model = new HomeViewModel
            {
                ...
                DiscountedBooks = _context.Books.Include(x => x.Author).Include(x => x.Images.Where(bi => bi.PosterStatus != null)).Where(x => x.DiscountPercent > 0).OrderByDescending(x => x.DiscountPercent).Take(5).ToList(),
            };
            return View(model);
        }
    }
